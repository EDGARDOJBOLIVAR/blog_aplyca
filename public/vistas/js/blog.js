...
const TEMPLATE_ENTRY_PREVIEW = '<h3 class="text-center">{TITULO}</h3><h2 class="text-center">{SUBTITULO}</h2><img src="'+RUTA_IMAGENES+'{IMAGEN}" class="img-thumbnail mx-auto d-block" style="max-width:60%" /><strong>{AUTOR}</strong>/<span>{FECHA}</span><p>{TEXTO}</p><a class="text-info" href="/ver-entrada/{ID}" title="Ver entrada completa">Ver todo</a>';
...
function view_entry_preview(element) {
    ...
    let Data = {
        ...
        SUBTITULO: $this.attr('entry_subtitle').toUpperCase(),
        IMAGEN: image,
